...
(
    ...
    first_prisoner_reason_code  VARCHAR(11) NOT NULL CHECK (length(first_prisoner_reason_code) > 0),
    ...
    second_prisoner_reason_code VARCHAR(11) NOT NULL CHECK (length(second_prisoner_reason_code) > 0),

    -- non-association details
    restriction_type_code  VARCHAR(8) NOT NULL CHECK (length(restriction_type_code) > 0),
