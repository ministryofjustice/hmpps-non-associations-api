#!/usr/bin/env bash
#
# Exports the non-associations schema as a flat CSV data dictionary.
#
# The descriptions come from the COMMENT ON statements in db/migration/V1_3__schema_comments.sql, so this
...
-- column of both. This schema has a single table so it cannot happen today, but it bit the incentives
-- API (see IR-1879). pg_constraint carries the owning relation, so it cannot.
LEFT JOIN (
